...
from .serializers import (
    ...
    DatasetQueriesSerializer, DatasetRelationshipSerializer
)
...
class DatasetViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['data_structure', 'growth_rate', 'access_patterns', 'current_datastore', 'query_complexity']
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(f"Attempt to update non-existent dataset with ID: {kwargs.get('pk')}")
            return Response(
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(f"Attempt to retrieve non-existent dataset with ID: {kwargs.get('pk')}")
            return Response(
                {'error': f'Dataset with ID {kwargs.get("pk")} not found.'},
                status=status.HTTP_404_NOT_FOUND
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            if related_queries > 0 or related_relationships_from > 0 or related_relationships_to > 0:
                logger.info(f"Deleting dataset '{dataset_name}' will also delete {related_queries} queries and {related_relationships_from + related_relationships_to} relationships")
            ...
            return Response(
                {
                    ...
                    'cascade_info': {
                        ...
                        'deleted_relationships': related_relationships_from + related_relationships_to
                    }
                },
            ...
            logger.warning(f"Attempt to delete non-existent dataset with ID: {dataset_id}")
            return Response(
                {
            ...
            logger.error(f"Unexpected error during dataset deletion (ID: {dataset_id}): {str(e)}")
            return Response(
                {
                    'error': 'An unexpected error occurred while deleting the dataset.',
                    'details': str(e) if settings.DEBUG else 'Contact administrator for assistance.'
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class DatasetQueriesViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(f"Query '{query_name}' (ID: {query_id}) from dataset '{dataset_name}' deleted successfully")
            ...
            logger.error(f"Unexpected error during query deletion (ID: {query_id}): {str(e)}")
            return Response(
                {
...
class DatasetRelationshipViewSet(viewsets.ModelViewSet):
    ...
    # Filtering and search
    filterset_fields = ['from_dataset', 'to_dataset', 'relationship_type', 'is_active', 'strength']
    ...
    def get_queryset(self):
        ...
        if dataset_id:
            queryset = queryset.filter(
                models.Q(from_dataset=dataset_id) | 
                models.Q(to_dataset=dataset_id)
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            instance = self.get_object()
            from_dataset_name = instance.from_dataset.name if instance.from_dataset else "Unknown"
            to_dataset_name = instance.to_dataset.name if instance.to_dataset else "Unknown"
            relationship_type = instance.relationship_type
            
            with transaction.atomic():
                self.perform_destroy(instance)
            
            logger.info(f"Relationship '{relationship_type}' between '{from_dataset_name}' and '{to_dataset_name}' (ID: {relationship_id}) deleted successfully")
            ...
            logger.warning(f"Attempt to delete non-existent relationship with ID: {relationship_id}")
            return Response(
                {
            ...
            logger.error(f"Unexpected error during relationship deletion (ID: {relationship_id}): {str(e)}")
            return Response(
                {
