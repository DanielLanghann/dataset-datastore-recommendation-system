...
class TestDataGenerator:
    def __init__(self, db_config):
        ...
        # Add hierarchy configuration
        self.hierarchy_config = {
            'max_depth': 4,              # Maximum hierarchy levels (0=root, 1=main, 2=sub, 3=leaf)
            'branching_factor': {        # How many children per level
            ...
            'level_probability': {       # Probability of creating children at each level
        ...
        # Predefined category hierarchy templates
        self.category_templates = {
            # Electronics hierarchy
            'Electronics': {
                'Mobile Devices': ['Smartphones', 'Tablets', 'Smartwatches', 'Accessories'],
                'Computers': ['Laptops', 'Desktops', 'Gaming PCs', 'Workstations'],
                'Audio & Video': ['Headphones', 'Speakers', 'TVs', 'Cameras'],
                'Gaming': ['Consoles', 'PC Games', 'Accessories', 'VR Equipment'],
                'Smart Home': ['Security', 'Lighting', 'Climate', 'Appliances']
            },
            # Fashion hierarchy
            'Fashion & Clothing': {
                'Men\'s Clothing': ['Shirts', 'Pants', 'Jackets', 'Underwear'],
                'Women\'s Clothing': ['Dresses', 'Tops', 'Bottoms', 'Intimates'],
                'Shoes': ['Athletic', 'Casual', 'Formal', 'Boots'],
                'Accessories': ['Bags', 'Jewelry', 'Watches', 'Belts']
            },
            # Books hierarchy
            'Books & Media': {
                'Technical Books': ['Programming', 'Data Science', 'Engineering', 'Math'],
                'Fiction': ['Mystery', 'Romance', 'Sci-Fi', 'Fantasy'],
                'Non-Fiction': ['Biography', 'History', 'Self-Help', 'Business'],
                'Educational': ['Textbooks', 'Reference', 'Children', 'Language']
            },
            # Home hierarchy
            'Home & Garden': {
                'Furniture': ['Living Room', 'Bedroom', 'Office', 'Outdoor'],
                'Kitchen': ['Appliances', 'Cookware', 'Utensils', 'Storage'],
                'Garden': ['Tools', 'Plants', 'Outdoor Furniture', 'Irrigation'],
                'Decor': ['Lighting', 'Wall Art', 'Textiles', 'Ornaments']
            },
            # Sports hierarchy
            'Sports & Fitness': {
                'Fitness Equipment': ['Cardio', 'Strength', 'Yoga', 'Accessories'],
                'Team Sports': ['Soccer', 'Basketball', 'Baseball', 'Football'],
                'Individual Sports': ['Running', 'Cycling', 'Swimming', 'Tennis'],
                'Outdoor Activities': ['Camping', 'Hiking', 'Fishing', 'Climbing']
            }
        }

    def generate_categories(self, count):
        """
        Generate hierarchical category data that scales from 1 to millions of records.
        
        Algorithm approach:
        1. First, create/maintain predefined hierarchies for realism
        2. Then generate additional categories to reach target count
        3. Maintain hierarchy ratios regardless of scale
        4. Handle incremental additions gracefully
        """
        
        # Get existing categories and analyze current structure
        existing_structure = self._analyze_existing_hierarchy()
        
        # Determine how many categories we actually need to create
        ...
        print(f"   📊 Current: {existing_structure['total_existing']}, Target: {count}, Creating: {categories_needed}")
        
        categories = []
        
        # Step 1: Complete predefined hierarchies if needed
        ...
        # Step 2: Generate additional categories to reach target count
        if categories_needed > 0:
